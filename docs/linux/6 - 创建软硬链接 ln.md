...
2. 在前端中，使用 `npm` 安装依赖，`node_modules` 中某个 package `package.json` 中的 `bin` 字段所指向的文件，会以软链接形式存放在 `node_modules/.bin` 目录。（`pnpm` 是生成脚本）     ![](./06/4.png)
3. 在前端中，`npm i -g` 全局安装，如果是有可执行文件的，会将 package 的 `bin` 字段所指向文件，在 `$PATH` 目录下以软链接形式存在，后续可直接调用。    ![](./06/5.png)
...
- 依赖真实的路径为 `/node_modules/.pnpm/<name>@<version>/node_modules/<name>`，其所需要的依赖（包含了非工程直接依赖的包），也都是存放在与其同级的文件夹中 `/node_modules/.pnpm/<name>@<version>/node_modules/`，且软链接形式存在，软链接到 `.pnpm/<name>@<version>` 。 ![](./06/6.png)
...
- `pnpm v5` 将创建一个“半严格”的 `node_modules`。项目将只能导入 `package.json` 中声明的直接依赖包，而依赖包能访问任何其他的包。[pnpm的node_modules配置选项](https://pnpm.io/zh/blog/2020/10/17/node-modules-configuration-options-with-pnpm)，`pnpm` 将所有包，软链接一份放在了 `/node_modules/.pnpm/node_modules` 下，这样就解决了依赖包找不到依赖包的问题。![](./06/3.png)
