...
```
指 `isOK(i)` 的返回值随着 `i` 的增加而增加，或者随着 `i` 的增加而减小，满足递增或递减有规律。

**这个逻辑和「在有序数组中查找一个元素」一样**，将 `num[i]` 当成函数，这便和 `isOK(i)` 一样了
```js
for (let i = 0; i < n; i++)  {
    if (nums[i] === target) {
        return answer;
...
var minEatingSpeed = function(piles, h) {
	...
	while (left <= right) {
		...
		//  	right = mid - 1 // [left, mid - 1]
		//}
		if (canFinish(mid, piles, h)) { // 相等和速度快都需要搜索右边以达到找左边界的目的
			right = mid - 1 // [left, mid - 1]
		} else {
			left = mid + 1 // [mid + 1, right]
		}
...
const canFinish = (speed, weights, days) => {
    let i = 0
    for(let nowDay = 1; nowDay <= days; nowDay++) {
        let remain = speed
