...
用户需要记住端口号，这个负担很沉重。通过 nginx 进行反向代理后，可以通过二级域名或者路径，直接访问不同的项目。而且 nginx 提供了许多静态服务器需要的基础功能，大大提高了开发效率。
...
2. 为什么要在响应头返回 content length：     
    ...
    没有设置content-length ，http 响应报文的 会有 Transfer-Encoding: chunked  字段，意思报文的body 部分不是一次性发过来的，而是分成 一个个快(chunk) 发送，主要用于大文件传输。它的编码规则规定 chunk 都有两部分组成 长度头 和数据包，相当于我们一次请求 多传输了 n 个长度头的数据     

    设置了Content-Length时，基于steam 的流式传输 也是分块传输 但是 没有分块长度头部分 。（之前一直以为设置和http 报文Content-Length 就不会分块）
    为了提高效率 更好 还是使用压缩 nginx gzip on 压缩 text/html 文本
    ![](./220727/3.jpg)


    设置了 content-length 后，stream 还是以分块的形式传播。当客户端读取Content-length的值大小的资源时，表示数据接收完毕。

    Content-Length 编码格式相比Chunk编码格式更小。chunked 每次还需要传输 length 和 \r\n 作为每一段的标识，最后使用长度为0的块作为数据发送完毕标志



    设置了 Content-Length 后，stream 还是会分块传输，只不过不是 transfer-encoding: chunked 这种编码格式。至于为什么还会计算 Content-Length，是因为它相比较 chunk 来说，编码格式会更小。因为 chunked 每次还需要传输 length 和 \r\n 作为每一段的标识。




### 6.3 继续完善静态服务器，使其基于 stream，并能给出正确的 Content-Length


### 6.4 继续完善静态服务器，使其作为一个命令行工具，支持指定端口号、读取目录、404、stream (甚至 trailingSlash、cleanUrls、rewrite、redirect 等)。可参考 serve-handler。


### 6.5 什么是 rewrite 和 redirect？
在客户端请求的资源不在本服务器的情况下
