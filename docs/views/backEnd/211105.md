...
- import/export：
    - ES6语法标准
    - 在编译时调用
    - 本质是结构过程，前面不允许有其他逻辑代码，需要babel转换为require。必须放在文件开头
...
结论：发展后，本质上还是打包成require/exports
...
也可以从另一个角度来理解：    
因为require/exports在运行时调用，所以普通类型在运行时已经取值了，后续对原变量的修改不影响之前的取值。    
import/export(ES6)在编译时调用，普通类型在每次调用时，生成只读引用，等到脚本真正执行时，才去取值。
...
*/
```
通过输出可得出，required取得的普通类型的值和原变量无关联，而import取得的普通类型的值与原变量息息相关（与只读引用，编译时调用有关）。
