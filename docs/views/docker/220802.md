...
1. ```nginx
    ...
    ```
2. 优先使用 brotli，不支持则使用 gzip。brotli 压缩文件比 gzip 小 20% 左右，解压速度也更快。
3. 使用 `try_files` 本质上也是去访问路由。
4. 正则匹配，走第一个路由，与写的顺序有关，如果第一个路由没有找到指定资源，也不会进入后续匹配的路由。
5. 即使删掉 `location /` 和 404 路由，也默认有这两个路由。
6. 502：网关错误，真实服务器节点没响应。504：网关超时，真实服务器节点超时，可能已过载，忙不过来。
...
2. 即使删掉 `location /` 和 404 路由，也默认有这两个路由。

3. 正则匹配，走第一个路由，与写的顺序有关，如果第一个路由没有找到指定资源，会进入后续匹配的路由吗？        
结论：不会。
    ```nginx
    location ~ /in {
      add_header X-Config A;
...
## 5. 应用场景
1. 强缓存：浏览器不会像服务器发送任何请求，直接从本地缓存中读取文件并返回         
    ...
    - Expires 是 HTTP1.0 的产物，Cache-Control 是 HTTP1.1 的产物，Cache-Control 优先级高于 Expires。在某些不支持 HTTP1.1 的环境下，Expires 就会发挥用外，现阶段 Expires 的存在只是一种兼容性的写法。
...
2. 协商缓存，向服务器验证是否过期。         
    ...
    协商缓存通过 Last-Modified 和 If-Modified-Since，或者 ETag 和 If-None-Match
    - 强制缓存若不生效则进行协商缓存
    - 协商缓存由服务器决定是否使用缓存
...
6. CORS:  跨域。 
    ...
    ```
7. HSTS : HTTP 严格传输安全（HTTP Strict Transport Security)。       
   ```nginx
    listen 443 ssl;
...
8. CSP： 内容安全策略CSP（Content-Security-Policy）, 可用于防范XSS注入   
...
docker 内部的服务发现，容器内可以通过 http//服务名（ service 名称作为 hostname） 访问，且直接使用容器内端口。            

2. 如果我宿主机起了个服务，在容器内，如何通过proxy_pass设置访问。         
在开发环境，可以更改 host 文件，设置 `host.docker.internal` 或者  `gateway.docker.internal` ，并在容器内部使用，但不适用于 除了 Docker Desktop 以外的生产环境。
    ...
    在实际生产环境中linux系统中，应该怎么设置？或者说 docker 不希望容器去访问宿主机的服务，不会有这种场景，有也是容器内到另一个 service？

1. yaml 的 version 3 是什么意思         
这代表我们定义的docker-compose.yml 文件内容所采用的版本，目前Docker Compose的配置文件已经迭代至了第三版，其所支持的功能也越来越丰富，所以我们建议使用最新的版本来定义。

4. 即使删掉 `location /` 和 404 路由，也默认有？
