...
- 如果依赖的依赖只有一组依赖项，将会以 `.pnpm/<name>@<version>` 形式存在。如果依赖的依赖有不同版本，将会以 `.pnpm/<name>@<version>_<subname>@<version>` 形式存在，在后面追加不同的依赖项版本做为唯一标志名（不同的依赖项可能是来自于子孙依赖)。      
  ...
  >  2. `.pnpm/<name>@<version>_<subname>@<version>` 后面追加不同的依赖项版本做为唯一标志名，不同的依赖项可能是来自于子孙依赖（这种场景是自依赖版本一致，但子孙依赖版本不一致）