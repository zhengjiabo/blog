...
3. `pacakge.json bin` 字段：npm 包命令行工具的入口，非常重要
4. `package.json main` 字段：
...
当找不到函数在哪里调用的时候，可以查找调用栈。在 VSCode 的调用栈模块按 `Ctrl + F` 搜索函数名。
...
run(callback) {
  ...
  const finalCallback = (err, stats) => {
    /* codes */
    if (callback !== undefined) callback(err, stats); // 断点后，根据调用栈找到的外层的代码。接下去思路就是顺着调用栈找 finalCallback 被谁调用
    /* codes */
  };
  ...
  const onCompiled = (err, compilation) => { // 打包完成
    ...
    compilation.endTime = Date.now(); // endTime 的负值
    const stats = new Stats(compilation); // stats 生成
    this.hooks.done.callAsync(stats, err => {
      if (err) return finalCallback(err);

      // 调用了 /* ----- 1 ----- */
      return finalCallback(null, stats);  // 把 stats 作为第二个参数传递 调用了
    });			
  }
...
### 4.2 webpack-cli 包的调用
  ...
  当然可以继续往下探究，`runCLI` 为什么传入 `['xx/bin/node', '/node_modules/.bin/webpack']` 就会执行 `/node_modules/.bin/webpack`     
  ...
  ```
  此时 `module` 为文本 webpack，所以通过 `require('webpack')` 就已经重新调用 `webpack` 包了，且调用的是 `webpack api`，即 `webpack` 包 `package.json` 的 `"bin"` 字段，即执行的是 `/modules/webpack/lib/index`。      

  结论已经有了。如果还要继续深入，可以继续往下看。
