...
### 3.4 docker push 将镜像推送到远程仓库
```sh
docker push nginx:alpine
```


### 3.5 docker inspect 查看镜像信息
可以看到 IPAddress IP地址等信息
```sh
...
### 6.1 `docker compose up` 构建/拉取镜像并启动容器。
配置yaml文件，可以基于已有镜像。
```yml
# docker-compose.yaml
version: "3"
