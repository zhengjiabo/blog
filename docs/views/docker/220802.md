...
### 3.2 即使删掉 `location /` 和 404 路由，也默认有这两个路由。
...
### 5.6 CORS:  跨域。 
    ...
    ```
### 5.7 HSTS : HTTP 严格传输安全（HTTP Strict Transport Security)。       
   ```nginx
    listen 443 ssl;
...
### 5.8 CSP： 内容安全策略CSP（Content-Security-Policy）, 可用于防范XSS注入   
