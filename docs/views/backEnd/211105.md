...
3. 后面不能跟值，例子3，本质还是将() => {}的值放在export后面
...
### 5.5 ES6的export，经bable转化成ES5, 打包后的源码对比
COMMONJS
```javascript
// module.js
const count = 0
...
### 5.6. import不同取值方式，打包后的源码对比
在5.5同一输出文件情况下，看import不同取值方式，打包后的源码对比
```javascript
// module.js
const count = 0
const func = () => {}
...
function(module, exports, __webpack_require__) {
    ...
    console.log('commonjs count');
    increaseCount(); // 原变量修改 单count已经复制完了，不影响count
    console.log('read count after increase in comonjs.js', count);
}
...
- 扩展：逗号操作符 (0, func)()    
    ...
    // 作用:import整个模块的所有属性时，会判断require的模块是否被babel处理过，返回模块引用。
    // 如果被处理过，正常返回。
    // 如果未被处理过，则浅拷贝一个新对象，且将原引用赋值在新对象的default属性，返回新对象
